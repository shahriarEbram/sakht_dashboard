def user_page(username, authenticator):
    from cons import work_shift, unit_name, work_type, categories
    ...
    with st.container(border=True):
        ...
        with col10:
            ...
            operation_dur_hour = st.number_input('ساعت', min_value=1, max_value=24, key="op_du_h")
            operation_dur_minute = st.number_input('دقیقه', min_value=0, max_value=60, key="op_du_m")
        with col11:
    ...
    if submitted_task:
        ...
        if not unit:
            ...
            st.session_state[df_key] = pd.concat([pd.DataFrame([new_row]), st.session_state[df_key]], ignore_index=True)
    ...
    task_df = st.data_editor(st.session_state[df_key],
                             disabled=['id', 'person_name', 'unit', 'shift', 'operation',
                                       ...
                                       'date', 'operation_duration', 'announced_duration', 'done_duration'],
    ...
    update_tasks(task_filtered_df, staff)
    st.session_state[df_key] = task_filtered_df
